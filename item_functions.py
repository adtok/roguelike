...
def cast_fireball(*args, **kwargs):
    ...
    if not libtcod.map_is_in_fov(fov_map, target_x, target_y):
        results.append({'consumed': False, 'message': 
            Message('You cannot target a tile that is outside the field of view!', libtcod.yellow)})
        return results 
...
def cast_confuse(*args, **kwargs):
    ...
    for entity in entities:
        if entity.x == target_x and entity.y == target_y:
            ...
            results.append({'consumed': True, 'message': 
                           Message(f"The eyes of the {entity.name} look vacant, as he starts to fumble around!", libtcod.light_green)})
